...
//Code Here
let postTaxPrices = prices.map((val, i, prices) => {
  return val * 1.07;
});
...
//Code Here
let totalPopulation = populations.reduce((total, val, i, populations) => {
  return total + val;
});
...
let ordersTotal = orders.map((val, i, orders) => {
...
// let bobsTotal = purchases.reduce((total, val, i, purchases) => {
//   if (val.owner === "Bob") {
